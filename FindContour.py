...
from lesson.lesson5 import on_trackbar
...
while True:
    ...
    # получим текущие значение ползунка
    lower_val = cv2.getTrackbarPos('Lower', 'image')
    upper_val = cv2.getTrackbarPos('Upper', 'image')

    edges = cv2.Canny(gray, lower_val, upper_val)  # создает изображение, на котором выделены контуры объектов
    ...
    # Перебирает все найденные контуры
    for countour in countours:
        rect = cv2.minAreaRect(countour)  # находит ограничивающий прямоугольник (bounding box) для контура
        # Ограничивающий прямоугольник - это прямоугольник минимальной площади,
        ...
        box = np.intp(box)  # преобразует массив координат углов
